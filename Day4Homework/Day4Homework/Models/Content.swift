...
class Content: Object, Mappable {
    ...
    static func create(content: Content) -> Content {
        let contentModel = Content()
        contentModel.id = content.id
        contentModel.name = content.name
        contentModel.address = content.address
        return contentModel
    }
    
    static func store(content: [Content]) {
        RealmService.shared.delete(dataType: Content.self)
        content.forEach {
            RealmService.shared.add(object: create(content: $0))
        }
    }
    
    static func fetchContent() -> Results<Content> {
        return RealmService.shared.fetch(dataType: Content.self)
    }
}
